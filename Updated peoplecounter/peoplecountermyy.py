...
parser.add_argument('-vid','--video',required=True,default="Input/cou.mp4",help="Video File Path")
parser.add_argument('-roi','--roi creation mode',required=True,default="manually",help="Create region of interest-do it 'manually'," + 
...
counter,textIn,textOut,cou,X,grpnum=0,0,0,0,0,0
...
def group_detection(x1,x2,y1,y2):
#    check,check1=0,1
    p1=[x1,x2]
    p2=[y1,y2] #of next box
    dist=np.sqrt(pow(p1[0]-p2[0],2)+pow(p1[1]-p2[1],2))#calculates distance
    if dist>=100 or (p2[0]==0 and p2[1]==0) or (p1[0]==0 and p1[1]==0):
        return 0
    elif (dist>0 and dist<58) :
        return 1
...
###        	    
            ...
            cou+=1
            pimg='Output_Images/'+videoname+'_person'+str(counter)+'.jpg'
            pimg1='Output_Images/'+videoname+'_person'+str(cou)+'.jpg'
            img=cv2.imwrite(pimg,rectf)
            imageset.append(rectf)
    ...
    cv2.putText(forig, "Groups: {}".format(str(grpnum)), (10, 70),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
